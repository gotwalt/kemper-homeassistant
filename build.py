...
HACS needs none of this — it downloads a release straight into
``custom_components/kemper``. The bundle is for installing by hand: a zip whose
paths are already relative to the configuration directory, so it unpacks where
it belongs, and ``--install`` for a config directory on this machine.

The bundle carries the integration alone. ``libkp`` is a requirement in the
manifest, and Home Assistant installs it from PyPI on first load.
